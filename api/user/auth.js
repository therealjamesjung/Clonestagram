...
// Reset profile API
router.put("/users/profile", middleware._auth, async (req, res) => {
  let query_response = {};
  const request_user = res.locals.user_id;
  const email = req.body.email;

  if (email && utils._validate_email(email) == false) {
  ...
  for (i = 0; i < body.length; i++) {
    set.push(`${body[i][0]}='${body[i][1]}'`);
  }

  if (set.length == 0) {
    return res.send(`No body has been given.`);
  }

  set = set.join(", ");

  try {
    await _query(`UPDATE User SET ${set} WHERE user_id='${request_user}'`);
    query_response.data = await _query(
      `SELECT user_id, email, name, bio, is_private FROM User WHERE user_id='${request_user}'`
    );
    query_response.message = `You have sucessfully reset your profile.`;
  } catch (error) {
    res.status(400);
    query_response.message = error;
  }
...
// Reset user_id API
router.put("/users/user_id", middleware._auth, async (req, res) => {
  let query_response = {};
  const request_user = res.locals.user_id;
  const user_id = req.body.user_id;
  ...
  try {
    await _query(
      `UPDATE User SET user_id='${user_id}' WHERE user_id='${request_user}'`
    );
    query_response.data = await _query(
      `SELECT user_id, email, name, bio, is_private FROM User WHERE user_id='${request_user}'`
    );
    query_response.message = `You have sucessfully reset your user_id`;
  } catch (error) {
    res.status(400);

    if (error.errno == 1062) {
      query_response.message = `Given user_id is already taken.`;
    } else {
      query_response.message = error;
    }
  }
